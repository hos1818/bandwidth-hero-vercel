...
async function compress(req, res, input) {
    try {
        ...
        let processedImage = prepareImage(sharpInstance, grayscale, isAnimated, metadata, pixelCount);
...
function getCompressionParams(req) {
    const format = parseBoolean(req.params?.webp, true) ? 'avif' : 'jpeg';
    const compressionQuality = Math.min(Math.max(parseInt(req.params?.quality, 10) || 75, 10), 100);
    const grayscale = parseBoolean(req.params?.grayscale, false);
...
function handleSharpError(error, res, sharpInstance, outputFormat, req, quality) {
    if (/too large for the HEIF format|unsupported file size/.test(error.message)) {
        ...
        const fallbackFormat = outputFormat === 'avif' ? 'jpeg' : 'webp';
        sharpInstance.toFormat(fallbackFormat, { quality })
            ...
            .catch((err) => {
                logError('Fallback format failed:', err.message);
                redirect(req, res);
            });
    } else {
