...
async function proxy(req, res) {
    ...
    try {
        const origin = await got(req.params.url, {
            method: 'GET',
            headers,
            timeout: { request: 10000 },
            followRedirect: true,
            maxRedirects: 5,
            responseType: 'buffer', // Ensures the response body is a buffer
            throwHttpErrors: false, // Similar to validateStatus in axios
        });
        ...
        if (error.response) {
            console.error('Server responded with status:', error.response.statusCode);
